...
const CURRENT_CACHE = "wurstbrot-1.0.0-stable-vt7";
...
test("fresh user precaches VT.7 and activates without an unnecessary reload", async () => {
...
test("existing VT.6 user switches immediately after the complete VT.7 precache", async () => {
  const url = "https://example.test/wurstbrot-suite/";
  const worker = createWorker({
    cacheNames: ["wurstbrot-1.0.0-stable-vt6", "other-app-cache"],
    clients: [url],
  });
  await worker.dispatch("install");
  await worker.dispatch("activate");
  ...
  assert.equal(worker.calls.skipWaiting, 1);
  assert.equal(worker.calls.claim, 1);
  assert.deepEqual(worker.deletedCaches, ["wurstbrot-1.0.0-stable-vt6"]);
